...
{
    [UsedImplicitly]
    internal class RequestStreamContent_WithContentLength_Tests : RequestContent_Tests
    {
        ...
        protected override HttpContent CreateHttpContent(byte[] data, int offset, int length, IPool<byte[]> pool)
            => CreateRequestContent(new StreamContent(new MemoryStream(data, offset, length), length), pool);
    }
}